...
job_body='{"name":"hex-'"$$"'","nodes":[{"id":"a","kind":"validate","param":"hello"}],"edges":[]}'
code="$(curl -sS -o /tmp/gogo-job-$$.json -w "%{http_code}" -X POST "$BASE/v1/jobs" -H 'Content-Type: application/json' -d "$job_body")"
if [[ "$code" != "201" ]]; then
  echo "[EXPECT] submit 201"
  echo "[ACTUAL] submit ${code}"
  exit 2
fi
...
echo "[EXPECT] GET /v1/jobs/{id} immediately after submit is 200"
echo "[ACTUAL] GET ${got}"
